...
public class FredCommon {
  ...
  // private static double getGrowth(String freq, double lastVal, double
  // nextLastVal, double factor) {
  //
  // double ret = 0.0;
  // try {
  ...
  // }
  //
  // private static double getGrowthValue(String freq, double lastVal, double
  // nextLastVal, double baseVal, double factor) {
  //
  // double ret = 0.0;
  //
  // System.out.printf("Growth : %s\t%f\t%f\t%f %n", freq, lastVal, nextLastVal,
  // baseVal);
  //
  // try {
  ...
  public static List<DataSeriesInfo> readSeriesNames(final String fname) {
    ...
    try (BufferedReader reader = new BufferedReader(new FileReader(fname))) {
      ...
      while ((line = reader.readLine()) != null) {
        final String str = line.trim();
        if (str.length() > 1) {
          if (!str.equals(FredCommon.infoHeader)) {
  ...
  public static String toFullFileName(final String series, final String title) {
    ...
    String ret = FredCommon.fredPath + "[" + series + "] - " + titl;
...
//	}
  ...
  public static String toSeriesInfo(final DataSeriesInfo dsi) {
    ...
    if (dsi != null) {
      ...
      ret = String.format("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s%n", dsi.getName(), dsi.getTitle(), "", dsi.getFrequency(), dsi.getUnits(), dsi.getType(),
          lastUpdate, lastObs);
    }
    return ret;
  }
  ...
  public static void writeSeriesInfo(final DataSeriesInfo dsi, final PrintWriter pw) {
    ...
    if (pw != null && dsi != null && dsi.getTitle() != null) {
      pw.printf("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s%n", dsi.getName(), dsi.getTitle(), dsi.getSeasonalAdjusted(), dsi.getFrequency(), dsi.getUnits(),
          dsi.getType(), dsi.getLastUpdate().toString(), dsi.getLastObservation().toString());
    }
  }
  ...
  public static void writeSeriesInfo(final List<DataSeriesInfo> dsList, final String filename) throws FileNotFoundException {
    ...
    try (PrintWriter pw = new PrintWriter(f)) {
      pw.printf("%s\t%s%n", FredCommon.infoHeader, dt.toFullString());
      for (final DataSeriesInfo ds : dsList) {
        // System.out.println(ds);
        try {
          if (ds != null) {
            pw.printf("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s%n", ds.getName(), ds.getTitle(), ds.getSeasonalAdjusted(), ds.getFrequency(), ds.getUnits(),
                ds.getType(), ds.getLastUpdate().toString(), ds.getLastObservation().toString());
          }
        }
        catch (final Exception e) {
          e.printStackTrace();
  ...
  // .replaceAll("Personal Consumption Expenditures", "PCE").replaceAll("London
  // Interbank Offered Rate", "");
  // final String s2 = s1.replaceAll("\\(", "").replaceAll("\\)",
  // "").replaceAll(", based on U.S. Dollar", "");
